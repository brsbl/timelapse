...
open_session() {
  "${BB_CLI:-bb}" browser-automation open --backend local --headless --machine "$HOST" --json | jq -r .id
}

start_page() {
  echo "$(date +%T) start $NAME frames $next..$TO on $HOST ($SID)"
  "${BB_CLI:-bb}" browser-automation run "$SID" --timeout 60s --json --script "
...
poll() {
  "${BB_CLI:-bb}" browser-automation run "$SID" --timeout 20s --json --script "
    const p = await browser.getPage('main');
...
while true; do
  ...
  if [ -z "$state" ] || echo "$state" | grep -q '"error"\|"prog":null'; then
    ...
    if [ "$failures" -ge 2 ]; then
      "${BB_CLI:-bb}" browser-automation close "$SID" --json >/dev/null 2>&1
      SID=$(open_session)
      start_page
      failures=0
    fi
