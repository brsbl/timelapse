...
async function rasterCaption(width: number, height: number, edit: TimelapseEdit, file: string, signal: AbortSignal) {
  const svg = captionSvg(width, height, edit);
  if (!svg) return false;
  await run("magick", ["-background", "none", "svg:-", file], signal, svg);
  return true;
}
  ...
  handlers: {
    ...
    exportMedia: async ({ root, path: relative, edit, kind, frame }, context) => {
      ...
      try {
        ...
        const hasCaption = await rasterCaption(info.width, info.height, edit, overlay, context.signal);
